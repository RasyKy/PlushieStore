...
export default function LoginForm({ onToggleForm }) {
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: process.env.NEXT_PUBLIC_DEV_SUPABASE_REDIRECT_URL || `${window.location.origin}/product`,
      },
    })
    ...
    <Card className="w-full max-w-md mx-auto shadow-lg border-0 bg-white">
      ...
      <CardContent className="space-y-6">
        ...
        <div className="text-center">
          ...
          <button type="button" onClick={onToggleForm} className="text-blue-600 hover:text-blue-700 font-medium">
            Sign Up
          </button>
        </div>
      </CardContent>
